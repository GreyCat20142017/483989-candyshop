...
(function () {
  ...
  var tabSwitchTo = function (className, container, classesObject) {
    Object.keys(classesObject).map(function (el) {
      ...
      var element = getElementBySelector(container, '.' + classesObject[item]);
  ...
  var onDeliverButtonsClick = function (evt) {
    ...
    if (el.name === 'method-deliver' && el.hasAttribute('id')) {
      tabSwitchTo(el.getAttribute('id'), deliverContainer, DELIVERY_CLASSES);
    }
    return false;
  };
  ...
  var onPaymentButtonsClick = function (evt) {
    ...
    if (el.name === 'pay-method' && el.hasAttribute('id')) {
      tabSwitchTo(el.getAttribute('id') + '-wrap', paymentContainer, PAYMENT_CLASSES);
    }
    return false;
  };

  var getXCoordinate = function (element) {
  ...
  var onPinMouseDown = function (evt) {
    var refreshSliderState = function (moveUpEvt) {
      ...
      var rangeLeft = getXCoordinate(rangeFilter);
      var sliderValue = getLimitedValue(newX, 0, rangeWidth) / rangeWidth * 100;

      currentPin.style.left = sliderValue + '%';
      var pinALeft = 100 * (getXCoordinate(rangePinA) - rangeLeft) / rangeWidth;
      var pinBLeft = 100 * (getXCoordinate(rangePinB) - rangeLeft) / rangeWidth;
    ...
    var sliderX = getXCoordinate(rangeFilter);
    var pinX = getXCoordinate(currentPin);
  ...
  var initCatalog = function () {
    ...
    if (paymentButtons) {
      paymentButtons.addEventListener('click', onPaymentButtonsClick);
    }
  ...
  var checkLuna = function (text) {
    var result = text.replace(' ', '').split('').map(function(item, i) {return ((i + 1) % 2 === 0) ? parseInt(item, 10) : parseInt(item, 10) * 2}).reduce(function(sum, current) {
  ...
  var paymentButtons = getElementBySelector(paymentContainer, '.payment__method');
